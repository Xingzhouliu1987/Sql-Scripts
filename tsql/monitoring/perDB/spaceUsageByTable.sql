...
GO
/*
	Breakdown of Space Usage by Each Physical Partition
*/
IF OBJECT_ID('adminTools.spaceUsageByFileVW') IS NOT NULL
   DROP VIEW adminTools.spaceUsageByFileVW;
GO
CREATE VIEW adminTools.spaceUsageByFileVW
AS
select 
	...
	SUM(files.autoGrowthMB) as autoGrowthMB
FROM 
    sys.tables t
INNER JOIN      
