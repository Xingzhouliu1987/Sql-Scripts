...
GO
IF OBJECT_ID('adminTools.diskFreeSpaceByFileFN') IS NOT NULL
   DROP FUNCTION adminTools.diskFreeSpaceByFileFN;
GO
/*
	File Size and Remaining Available Storage
*/
CREATE FUNCTION adminTools.diskFreeSpaceByFileFN(@database_id SMALLINT = -1)
 RETURNS TABLE
 AS
...
CONVERT(INT,(CASE 
	...
	ELSE 2
END)) AS autoGrowthMB
FROM (SELECT * FROM sys.master_files WHERE @database_id = -1 OR database_id = @database_id) mf
CROSS APPLY sys.dm_os_volume_stats(mf.database_id, mf.FILE_ID) dovs
GO


SELECT * FROM adminTools.diskFreeSpaceByFileFN(-1)